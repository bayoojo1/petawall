...
class StripeManager {
    ...
    private function syncSubscriptionFromStripe($subscriptionId) {
        try {
            ...
            error_log("Syncing subscription: " . $subscription->id);

            $userId = $subscription->metadata->user_id ?? null;
            $plan   = $subscription->metadata->plan ?? null;

            if (!$userId) {
                error_log("Missing user_id in subscription metadata");
                return;
            }

            if (empty($subscription->items->data)) {
                error_log("No subscription items found");
                return;
            }
            ...
            if (!isset($item->current_period_start) || !isset($item->current_period_end)) {
                error_log("Missing period dates from Stripe subscription");
                return;
            }
        ...
        switch ($status) {
            ...
            case 'trialing':
                ...
                if ($roleId) {
                    ...
                    error_log("User $userId upgraded to role $roleId");
                }
                break;

            case 'past_due':
                // Do NOT downgrade immediately
                error_log("User $userId subscription past_due");
                break;

            case 'canceled':
            case 'unpaid':
            case 'incomplete_expired':
                $this->updateUserRole($userId, 1); // free
                error_log("User $userId downgraded to free");
                break;
        }
    }
